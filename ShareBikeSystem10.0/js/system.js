...
//		dot = drawDot(e.clientX + 'px', e.clientY + 'px', 'regular');
		x = e.clientX + document.body.clientLeft - g_left;
		y = e.clientY + document.body.clientTop - g_top;
		bike = getBike(x, y, 'regular');
		addBike(bike);
		//document.getElementById("global").appendChild(bike);//IE6不兼容
		alert("停车成功！");
	} else if (e.clientX >= g_left && e.clientX <= g_right && e.clientY >= g_top && e.clientY <= g_bottom){
		//在地图内的非指定地点停放
		x = e.clientX + document.body.clientLeft - g_left;
		y = e.clientY + document.body.clientTop - g_top;
		handleIllegalBike(x, y);
	}
}
...
//在单车停放地点画一个单车图标，flag单车是否停在指定地点的标志，方便后来清场【老方法会错位】
function drawDot(x, y, flag) {
	var div = "<div id='bike' class='"+ flag +"' style='position: absolute;left: "+(x)+";top: "+(y)+";width: 30px;height: 20px;'></div>";
	return div;
}

//获取一辆新的单车，flag单车是否停在指定地点的标志
function getBike(x, y, flag) {
	//**********************除IE以外的浏览器支持***********
	/*bike = document.createElement("div");
	bike.setAttribute("id", "bike");
	bike.setAttribute("class", flag);
	style = "position: absolute;left: "+(x - 15)+"px;top: "+(y - 40)+"px;width: 30px;height: 40px;";
	bike.setAttribute("style", style);*/
	//***************另外一种写法***********************
	/*bike.id = 'bike';
	bike.class = flag;
	bike.style = "position: absolute;left: "+(x - 15)+"px;top: "+(y - 40)+"px;width: 30px;height: 40px;";*/
	var bike = "<div id='bike' class='"+ flag +"' style='position: absolute;left: "+(x - 15)+"px;top: "+(y-40)+"px;width: 30px;height: 40px;'></div>";
	return bike;
}

//将单车添加到指定位置
function addBike(bike) {
	globalELe = document.getElementById("global");
	globalELe.innerHTML += bike;
}
